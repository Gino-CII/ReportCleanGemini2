...
export default function App() {
  ...
  const formatExcelDate = (val: RawCell): string => {
    if (!val) return '';
  ...
  const parseNumeric = (val: RawCell): number => {
    ...
    try {
      ...
      for (let i = headerIdx + 1; i < rawRows.length; i++) {
        ...
        for (const cell of row) {
          ...
          const containsSubtotalKeyword =
            cellText.includes('小計') ||
            cellText.includes('合計') ||
            cellText.includes('總計') ||
            lowerCellText.includes('total') ||
            lowerCellText.includes('subtotal');

          if (!containsSubtotalKeyword) continue;

          isSubtotal = true;
          if (cellText.includes('小計') || lowerCellText.includes('subtotal')) {
            subtotalType = '小計';
          } else if (cellText.includes('總計') || lowerCellText.includes('grand total')) {
            subtotalType = '總計';
          } else {
            subtotalType = '合計';
          }
        ...
        for (let colIdx = 0; colIdx < row.length; colIdx++) {
          ...
          if (colIdx === dateColIdx && cellVal !== '') {
            const currentText = String(cellVal);
            const formatted = formatExcelDate(currentText);
        ...
        if (custColIdx !== -1 && cleanedCells[custColIdx]) uniqueCusts.add(String(cleanedCells[custColIdx]));
        if (prodIdColIdx !== -1 && cleanedCells[prodIdColIdx]) uniqueProds.add(String(cleanedCells[prodIdColIdx]));
        if (salesColIdx !== -1 && cleanedCells[salesColIdx]) uniqueSales.add(String(cleanedCells[salesColIdx]));
  ...
  const processExcelFile = (file: File): void => {
    ...
    reader.onload = (): void => {
      try {
        if (!(reader.result instanceof ArrayBuffer)) {
          throw new Error('檔案讀取結果不是有效的 ArrayBuffer。');
        }
        ...
        const message = error instanceof Error ? error.message : '未知的 Excel 解析錯誤';
    ...
    reader.onerror = (): void => {
      ...
      alert('❌ 檔案讀取失敗，請確認檔案未損毀且瀏覽器具有讀取權限。');
    };
              ...
              <div className="text-center space-y-4">
                ...
                <div>
                  <p className="font-semibold text-slate-700 text-lg">
                    {loading ? '正在解析檔案…' : (<>拖曳檔案到此處，或 <span className="text-indigo-600 underline">點擊瀏覽檔案</span></>)}
                  </p>
